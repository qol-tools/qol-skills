...
- Preview copy and copy-path actions are gated on the saved PNG, so the full-resolution encode latency is user-visible copy latency. Keep the PNG codec dependency chain in the workspace dev profile's `opt-level = 2` list; an unoptimized codec makes dev-build copies land hundreds of milliseconds late.
