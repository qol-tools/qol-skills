...
# hookSpecificOutput JSON format. Failures must be silent — a broken hook
# must never block a session.

set -uo pipefail

CONTEXT="qol-tray dev-restart cheat sheet (load before any qol-tray daemon analysis):
...
printf '{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":%s}}\n' \
    "$(printf '%s' "$CONTEXT" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read()))' 2>/dev/null || printf '"%s"' "$CONTEXT")"
