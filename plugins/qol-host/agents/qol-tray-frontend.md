...
- **Modifier-keyed alternates go through `Surface`, never per-view branching.** Shift/Ctrl/Meta + Enter or Click are handled centrally by the modifier→slot table in Surface. Declare `onSecondaryActivate` (or `actions[]` from `ui/lib/surface-actions.js`) on the surface and `data-secondary-label` for the overlay; never `if (event.shiftKey)` inside a view's handler. Click→dive is also Surface's responsibility, not the keyboard router's.
- **Destructive actions go through `<ConfirmButton confirmWith="...">`, never `window.confirm()`.** Native dialogs break visual language; the trait morphs the button into a contextual-keyword input. Pick a word that actually describes the action (`"restore"`, `"delete"`, `"disconnect"`).
