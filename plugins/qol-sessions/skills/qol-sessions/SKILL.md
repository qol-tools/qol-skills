...
| `session_wait_output(session, timeout_ms?, expect?)` | Block until the screen settles or contains an expected substring; returns settled, screen, polls, elapsed_ms |
...
- MCP clients (Claude Code, codex, kimi): `qol sessions mcp` over stdio exposes the same five tools; register it as a stdio MCP server.
