...
| `qol sessions compact <session>` (CLI) | Submit the target tool's own compaction command to one live session, report the context used before and after, and refuse with a typed reason; never delivers work |
| `session_compact(session)` | Agent surface for the same architect-owned control action on a lane |
...
`session_bridge` owns completion signalling, collection, and recovery while a round remains open. A recovered response returns `submitted=false`; review it before delivering anything further. Pass the reviewed response's `completion_marker` as `acknowledge_marker` on the next `session_submit`. A completed spawned lane instead closes after wake delivery; its checkpoint remains available for acknowledgement by `session_loop_close`, using the marker returned by the delivery call. The generated completion marker is split in the delivered prompt, so the target's input echo cannot complete the round. The lane's visible reasoning can still reconstruct the joined marker mid-turn, so completion is verified in the transcript, never in on-screen thinking. A round-complete wake means ready for architect review; it never means the feature is accepted.

Parallel lanes cost nothing extra: every delivery returns in seconds, so spawn or submit all lanes in one turn, end the turn, and review each lane's wake report as it arrives. A submit refuses when a round is already pending on that session; use `session_bridge` only when the wake leaves that round open.
...
Every lane reports its context used against its maximum. `sessions_list` carries it per session as `context` (`tokens`, `context_window`, `percent`, `source`), a round result carries the target's `context` from the moment the round was produced, and a completion wake names it as `Context: <used>/<window> tokens (<percent>%)`. `source` is `harness` when the tool itself reported the numbers, `transcript` when they were derived from its provider usage, and `unknown` when the tool cannot report usage; never present a derived number as a harness measurement. A tool reports through its own strategy: pi's reads the `qol-context` transcript entries its extension writes from `ctx.getContextUsage()` and falls back to the transcript's provider usage when the extension is absent, while a tool whose strategy exposes no usage source reports `unknown`.

`qol sessions compact <session>` (agent surface `session_compact`) submits the tool's own compaction command as its own submitted line - the command the tool's strategy exposes, or a refusal when it exposes none - then reports the context used before and after plus the saving. It is an architect-owned control action, not round delivery: it sends only that fixed command, never caller text, and it never opens, closes or leaves behind a bridge checkpoint. The at-prompt check reconciles the harness transcript with the lane's live screen, so a busy lane is refused even when its transcript cannot be resolved. It refuses with a typed reason instead of acting: `round_open` when a round is open or another bridge process is attached, `not_at_prompt` when the lane is inside a turn, `state_unknown` when the lane cannot prove it is at its prompt, `unsupported_tool` when the tool has no verified compaction command, `nothing_to_compact` when the harness reports the compactable span is already inside its retention budget, and `delivery_failed`, `compaction_failed`, `session_gone` or `timeout` when the action itself failed.

Auto-compaction is opt-out and runs before a round would resend the history. A spawned lane whose reported context reaches the threshold is compacted first, and the round result carries an `auto_compact` report naming the decision: `compacted`, `not_needed`, `opted_out`, `unsupported`, `usage_unknown`, `not_at_prompt`, `state_unknown` or `failed`. It never fires for a session the spawn surface did not launch, and never when usage is unknown, so a tool that cannot report usage behaves exactly as it did before. A lane resumed by `session_spawn` is checked before launch against its own saved transcript: when that transcript is over the threshold, the lane starts without its task, compacts at its prompt, then receives the task, and the spawn result carries the `auto_compact` report; that spawn call waits for the compaction.

`sessions.toml` owns the config surface: `auto_compact` (default `true`) and `auto_compact_percent` (default `80`, greater than 0 and at most 100). One lane opts out with `qol sessions spawn --no-auto-compact` (agent surface `session_spawn` with `auto_compact: false`), which is recorded in the spawn ledger and survives a respawn; `auto_compact = false` disables it for every lane.
...
4. On a completed spawned-lane wake, review the durable report named by the wake; do not call `session_bridge`. Retain the delivery call's `completion_marker` for `session_loop_close`. When the wake leaves the round open, collect it with `session_bridge` (no task); if that recovers prior output with `submitted=false`, inspect the output first and acknowledge its marker on the next `session_submit` or loop close.
...
6. If the feature is not accepted, respawn an auto-closed lane with the same key and a bounded correction task; keyed spawn resumes its harness history. For a round whose session remains live, use `session_submit` with the reviewed marker. Then return to step 3.
...
`qol sessions watch <tokens>` waits on pending rounds, prints one JSON event per line (`completed` when the completion marker appears, `gone` when the terminal died with the round open and the round is then discarded, `completed_markerless` when the lane resolved without a matchable marker), and exits 0 when no watched rounds remain pending. For pi lanes the marker is verified in the pi transcript's latest assistant text block only, never in on-screen thinking: the lane must reconstruct the split marker in its reasoning, which the TUI renders on screen, so a screen match would fire mid-turn and autoclose the lane while it is still working. Other tools keep the screen grep. Polling starts at a 3s base interval and backs off to a 30s cap; every 10th read is a strict screen read, the rest are relaxed. A lane whose window closed right after showing its completion marker completes with its last screen instead of going `gone`, so the report survives the lane process exiting.

The marker match is tolerant: after the `QOL_BRIDGE_DONE_` prefix, non-alphanumeric characters are stripped (a wrapped or reflowed token still matches), and the token itself is accepted when it shares the first 10 hex characters exactly and stays within 3 Levenshtein edits of the expected token. A mistyped marker therefore completes the round normally instead of silently killing it; a different lane's marker and the split-fragment prompt echo can never match by accident.
...
Never infer lane state from transcript growth, file freshness or screen movement. A turn in flight writes nothing for minutes: measured zero-write stretches inside a single live turn reached 260s, including one 4m36s pure-think gap, so a thinking lane reads as finished. Writes are bursty, so no polling interval rescues it. The completion marker is matched in the pi transcript's latest assistant text for pi lanes (thinking blocks and tool output never count) and on the screen for other tools. A round that was observed Working and then reads Ready for 3 consecutive polls without a matchable marker completes markerless with its screen tail attached, so a lane that finished but never printed the marker surfaces within seconds; the 15-minute screen-change timeout remains the fallback for harnesses without a deterministic runtime.
...
The architect reviews a completed spawned lane from the report carried by its wake; the lane and pending checkpoint have already closed, so `session_bridge` has nothing to collect. A non-closing completion wake still uses `session_bridge` (task omitted). A `gone` wake means the lane terminal closed and its round was discarded; start a fresh lane only if the work still matters. A `stalled` wake means the target went idle; nudge it with `qol sessions resume --kickstart`, or collect with `session_bridge`.
...
A completed spawned-lane wake followed by `session_bridge` returning "no pending bridge" is not a delivery failure: the wake report is the receipt and the watcher intentionally closed the checkpoint. Review the named report and do not resubmit the completed task.
...
6. `session_compact` is a control action. It submits only the tool's fixed compaction command, never a task, a relayed message, or caller-authored text, and compaction is never a route for delivering work to a lane.
