...
const TIER_RULE = [
    ...
    'Every implementation, research, and preliminary-review lane is spawned with session_spawn carrying tool: "pi", background:true, and a model from the sessions.toml allow list (spawn_model is the default; allowed_models names every model this host may launch, and the spawn refuses anything unlisted); the harness default, a missing model, or a foreground round that suspends the architect in an open tool call is a refusal point, never a silent choice.',
    'Never spawn a Claude Code lane: tool "claude" is forbidden on this host no matter which harness the architect itself runs in, and pi is the only permitted lane tool; the lane model always comes from the sessions.toml allow list, never from a name in your own context.',
...
const BRIDGE_CONTEXT = [
    ...
    'Spawned lanes always run tool "pi" with a model from the sessions.toml allow list (spawn_model is the fallback when an override is absent, allowed_models names every model this host may launch); tool "claude" is never spawned, and a lane that came up on the wrong tool or an unlisted model is closed and respawned before any work is bridged.',
