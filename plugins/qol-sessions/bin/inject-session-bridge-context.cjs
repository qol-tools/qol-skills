...
const TIER_RULE = [
    ...
    'Every implementation, research, and preliminary-review lane is spawned with session_spawn carrying tool: "pi", background:true, and a model that the sessions.toml tool_models mapping declares for pi (spawn_model is the default; allowed_models is the spending allowlist; a tool/model pair that tool_models does not declare is refused); the harness default, a missing model, or a foreground round that suspends the architect in an open tool call is a refusal point, never a silent choice.',
    'Never spawn a Claude Code lane: tool "claude" is forbidden on this host no matter which harness the architect itself runs in, and pi is the only permitted lane tool; the lane harness and model pairing always come from the sessions.toml tool_models mapping, never from a name in your own context.',
...
const BRIDGE_CONTEXT = [
    ...
    'Spawned lanes always run tool "pi" with a model that the sessions.toml tool_models mapping declares for pi (spawn_model is the fallback when an override is absent; allowed_models is the spending allowlist; an undeclared tool/model pair is refused); tool "claude" is never spawned, and a lane that came up on the wrong tool or an unlisted model is closed and respawned before any work is bridged.',
