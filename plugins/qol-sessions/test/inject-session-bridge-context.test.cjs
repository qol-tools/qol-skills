...
test('the skill requires an event-driven review loop through feature acceptance', () => {
    ...
    assert.match(skill, /never call `session_bridge` speculatively/);
    assert.match(skill, /Never poll a process, continuation handle, screen, session, status, or clock/);
    assert.match(skill, /A round-complete wake means ready for architect review; it never means the feature is accepted/);
