...
- No comments — code must be self-explanatory. This is a hard rule, not a default.
  ...
  - Enforced by the tldr-agent plugin (kmrh47-skills marketplace, skill `/tldr-agent:no-code-comments`): a PreToolUse hook denies newly added comments in Edit/Write/MultiEdit in every repo, and a UserPromptSubmit hook reports comments added by any other write path before the next turn is written. Bypass is the reader's call: one-shot `touch .claude/bypass-no-comments`, or permanent per-repo `touch .claude/allow-code-comments`. Applies only to source languages (`.rs .js .mjs .cjs .jsx .ts .tsx .mts .cts .go .java .c .h .cc .cpp .hpp .cs .kt .kts .swift .dart .php .css .scss .less .py .sh .bash .zsh .rb`); Markdown, JSON, YAML, TOML, and HTML are out of scope.
  - When a comment is genuinely warranted (an upstream file that must keep its licence header, a generated file, a directive comment such as `# noqa` or `// eslint-disable`), ask the user, then run `touch .claude/bypass-no-comments` in the repo root. The hook consumes the marker on the next denial and re-locks itself.
