...
- No comments — code must be self-explanatory. This is a hard rule, not a default.
  ...
  - Enforced by the `code-deny-comments` PreToolUse hook on Edit / Write / MultiEdit. It compares the comments in the replacement text against the comments in the text being replaced, so moving or keeping an existing comment passes and introducing a new one is denied. It fires only for paths inside a `qol-*` clone or worktree, and only for source languages (`.rs .js .mjs .cjs .jsx .ts .tsx .mts .cts .go .java .c .h .cc .cpp .hpp .cs .kt .kts .swift .dart .php .css .scss .less .py .sh .bash .zsh .rb`). Markdown, JSON, YAML, TOML, and HTML are out of scope.
