...
- No comments — code must be self-explanatory. This is a hard rule, not a default.
  ...
  - Enforced by the tldr-agent plugin (kmrh47-skills marketplace, skill `/tldr-agent:no-code-comments`) on Claude Code and pi: a PreToolUse hook denies newly added comments in Edit/Write/MultiEdit in every repo, and a UserPromptSubmit hook reports comments added by any other write path before the next turn is written. (Codex and kimi are not yet wired to the plugin; the rule text still applies to them, the hooks do not.) Bypass is the reader's call, and the reader creates the marker: one-shot `touch .claude/bypass-no-comments`, or permanent per-repo `touch .claude/allow-code-comments`. The hooks enforce the same source-language scope as the skill's Scope section; Markdown, JSON, YAML, TOML, and HTML are out of scope.
