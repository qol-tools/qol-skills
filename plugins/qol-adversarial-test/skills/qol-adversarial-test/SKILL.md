...
14. **Productize stable work.** Extend the repository's native orchestrator before creating a parallel runner. Make one human command reproduce the storm and emit a machine-readable report. Apply the `qol-workflow-nodes` skill when available.
