...
log() { printf '[plugin-lights/route-to-agent] %s\n' "$*" >&2; }
...
# Only guard files inside the plugin-lights repo.
case "$PATH_ARG" in
    *"/plugin-lights/src/"*|*"/plugin-lights/ui/"*|*"/plugin-lights/tests/"*|*"/plugin-lights/"*"plugin.toml"|*"/plugin-lights/"*"qol-config.toml"|*"/plugin-lights/"*"qol-runtime.toml"|*"/plugin-lights/"*"Cargo.toml")
        AGENT="qol-plugin-lights:plugin-lights"
        ;;
    *) exit 0 ;;
esac
...
cat >&2 <<EOF
Edit to $PATH_ARG is blocked: plugin-lights scope must route through its specialist agent.
