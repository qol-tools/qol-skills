#!/usr/bin/env node
/*
 * SubagentStop hook: after a plugin-alt-tab agent finishes, ask Claude
 ...
 * but scoped to the plugin-alt-tab agent only.
 *
 * Runs headlessly. All errors are silent.
 */
...
const ELIGIBLE_AGENTS = new Set([
    'plugin-alt-tab',
    'qol-plugin-alt-tab:plugin-alt-tab',
]);
...
function log(msg) {
    process.stderr.write(`[plugin-alt-tab/reflect-agent] ${msg}\n`);
}
