...
function piExtensionContent(root, pluginName, failures) {
  ...
  lines.push("      updatedPrompt = decision?.updatedPrompt;");
  lines.push("    } catch (_ignored) {}");
  lines.push("  }");
  lines.push("");
  lines.push("  return { blocked: false, context, systemMessage, updatedPrompt };");
  lines.push("}");
  lines.push("");
  ...
  if (preToolUse.length > 0) {
    ...
    lines.push("      }");
    lines.push("");
    lines.push("      for (const hook of PRE_TOOL_USE_HOOKS) {");
    lines.push("        const matched = hook.matcher");
    lines.push("          && matchedToolName(hook.matcher, event.toolName);");
    lines.push("");
  ...
  if (userPromptSubmit.length > 0) {
    ...
    lines.push("      let prompt = event.text;");
    lines.push("");
    lines.push("      for (const hook of USER_PROMPT_SUBMIT_HOOKS) {");
    lines.push('        const input = JSON.stringify({ cwd: ctx.cwd ?? "", prompt });');
    lines.push("        const result = runHook(hook.script, input);");
    lines.push("");
    lines.push("        if (result.blocked) {");
    ...
    lines.push("        }");
    lines.push("");
    lines.push('        if (typeof result.updatedPrompt === "string") {');
    lines.push("          prompt = result.updatedPrompt;");
    lines.push("        }");
    lines.push("");
    ...
    lines.push("");
    lines.push("      if (prompt !== event.text) {");
    lines.push('        return { action: "transform", text: prompt };');
    lines.push("      }");
    lines.push("    });");
    lines.push("");
